...
public:
	...
	const int& getIDQuest() const {
		return ID_;
	}

	const string& getQuest() const {
		return question.quest_;
	}
	const string& getAnswer() const {
		return question.answer_;
	}
	const std::vector<string>& getReply() const {
...
public:
	QuestUser(const Quest& quest) : Quest(quest.getIDQuest(), quest.getQuest(), quest.getAnswer()) {
		question.reply_ = std::move(quest.getReply());
	}
	QuestUser(int id, string quest, string answer, bool answer_given, int answers, double grade)
		: Quest(id, quest, answer), answer_given_(answer_given), answer_(answers), grade_(grade) {};
	...
	void saveQuest() {
		...
		if (file.is_open()) {
			file << ID_ << ":::" << question.answer_ << ":::" << question.quest_ << ":::" << answer_given_ << ":::" << answer_ << ":::" << grade_;
			for (auto i : question.reply_) {
				file << ":::" << i;
			}
			file << "\n";
